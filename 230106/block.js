...
class Block extends IBlock {
  ...
  //블록 생성 함수
  create(_previosBlock, _adjustmentDifficulty, _adjustmentTimestamp, _data) {
    try {
      ...
      this.difficulty = this.getDifficulty(
        //난이도를 구하는 메서드 호출
        {
          ...
          previousDifficulty: _previosBlock.difficulty,
    ...
    return Array.isArray(_data)
      ? {
          ...
          value: _data.length
            ...
            : //merkle 라이브러리를 사용하며 sha256 방식의 hash 암호화 , merkleRoot를 구한다
              "0".repeat(64),
  ...
  //난이도 구하는 메서드
  //매개변수안에 객체로 묶어서 나중에써줄때 몇개만써도 불러올수있음
  getDifficulty({
    height, //입력되는 높이
    timestamp, //입력되는 시간
    ...
    //높이 난이도 조절 단위 개수 나머지가 0이 아니면 개수가 맞아 떨어지지 않아서
    if (height % DIFFICULTY_ADJUSTMENT_INTERVAL !== 0)
      //이전 난이도를 이용
      return previousDifficulty;
    //만약 난이도 조절 단위 개수가 10이라면 10개 단위로 난이도 조절을 하게된다.
    //높이가 9까지는 0,높이가 10~19까지는 1, 이후에는 각 10단위마다 아래코드로 난이도를 결정하게된다

    //20,30,40,50부터
    const timeTaken = timestamp - adjustmentTimestamp;
    // 현재 난이도 조절 개수 단위의 블록 생성 시간과 이전 난이도 조절 개수 단위의 블록 생성 시간의 차를 확인
    //블록이 생성된 시간 - 10개 이전의 생성시간=시간차(난이도를 올리거나 내리거나 난이도조절을위해 )
    ...
    //설정된 블록 10개 생성 시간보다 0.5배 미만이면 난이도 증가 , 300초보다 작은경우
    ...
    //위 두 조건이 맞지 않으면 기존 난이도
  }

  // 블록 업데이트 블록 마이닝(채굴)
  updateBlock(_previosBlock, _adjustmentDifficulty, _adjustmentTimestamp) {
    //난이도에 맞게 hash를 생성, 문제풀이라고도 함
    let hashBinary = hexToBinary(this.hash);
    //최근블록의 hash
    //현재 hash를 bit형식으로 , binary 형식으로 바꾼다
    //startwith 특정 문자시작하는거 확인하는 메서드
