...
2. external 필수 (외부에서 이더를 보낼수있으니 external붙인다)
3. payable 필수 (이더 받고 주고 이런건 payable붙여야한다)
...
//     }
    ...
    fallback() external payable {
     ...
     // 스트링 보여주는걸 지우면 가스 2300안넘어서 send와 transfer은된다
    }
...
contract You{
    ...
    //receve() 
    // send , transfer는 가스 소비가 적어서 실패하고 call만 성공한다 2012 이후 call권장
