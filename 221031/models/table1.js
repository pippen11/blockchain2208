...
//   static init(sequelize) {
//     // 테이블 생성
//     //왜 static으로적은지 알아야한다 이런거할땐 static을쓴다 앞에붙여야한다
//     return super.init(
//       {
//         // 컬럼들을 작성한다.
//         column1: {
//           type: Sequelize.STRING(10),
//           //이건 static이다
//           //type을 적을때 왜 Sequelize에서 가져올까?
//           // number, string << 자바스크립에잇는데 왜 가져올까?
//           // javascript에 숫자가 들어왔을때 정수인지 실수인지 알수있을까?>> 없다 >>DB에서는 알아야한다
//           // DB에서 사용하는 자료형에 맞게 javascript의 자료형을 넣어주기위해서 Sequelize 라이브러리에서 제공하는 자료형을 사용한다
//         },
//         column2: {
//           // type:Sequelize.NUMBER// << 얘는 index로 사용하지 못한다.
//           // INT가 아니라 숫자다 , FLOAT / DOUBLE 둘중 하나일거다
//           type: Sequelize.INTEGER.UNSIGNED,
//           //UNSIGNED는 뭐냐? 0 < number << 무조건 양수 음수가없이 0과 양수로만 이루어져있다
//           // -20억 ~ 20억 / 0~40억 << 용량은 그대로이기 때문에 숫자를 원하는 만큼 더 사용할수잇다
//           primaryKey: true, //테이블당 하나만 가능하다, 검색에 용이하다.
//           //
//           unique: true, // 데이터가 중복될수없다.
//           autoIncrement: true, // 자동 증가 //인덱스가 자동증가?
//         },
//       },
//       {
//         //테이블에 대한 기본 설정
//         sequelize,
//         timestamps: true, //create_at , update_at을 자동으로 추가한다.
//         underscored: true, // 카멜을 스네이크로 바꾼다. << CreateAt-> create_at
//         paranoid: false, // delete_at을 추가한다.(false면 추가안함 )
//         //데이터를 삭제했을때 DB에서 아예 없앨건지 , 아니면 남길건지 결정해라 << true면 남긴다 ->: delete_at이 추가한다
//         modelName: "Table1", // javascript에서 사용하는 이름
//         tableName: "table1", // DB에 생성되는 테이블 이름
//         charset: "utf8mb4",
//         collate: "utf8mb4_general_ci",
