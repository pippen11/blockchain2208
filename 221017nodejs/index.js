...
//콜스택:함수실행 이벤트큐: 마우스클릭 메모리힙 : 변수선언->자바스크립트
// 이벤트 기반 : 이벤트 루프가 있어 정의된 이벤트에 대해서 코드를 실행한다.->자바랑 거의동일
//싱글 스레드
// 프로세스랑 스레드
...
// 다만, 멀티 스레드 기능을 사용할수있게 해주는 라이브러리가 있다.

// 논 블로킹 I/O
// I/O -> In Out
// 논브로킹? -> 동기 /비동기, 블로킹/논브로킹
// 동기: 실행되는 작업이 서로 연관성을 가진다.
//-> 하나가 실행이 완료된 후에 그 반환값을 사용해 다른 작업을 실행한다
// 이전 작업을 기다려야한다.
...
//console 객체의 log메서드를 실행하기 위해 add함수를 실행한다
//add가 끝나야 console log 메서드를 실행한다.
//전형적인 동기방식
//비동기 : 작업을 실행하는데 별 관계가 없다. -> 함께 실행될수있다.
// 이벤트 함수들
console.log(a);
setTimeout(() => {
  console.log(b);
...
//블로킹 / 논브로킹
// 블로킹 : 동기작업은 이전작업을 기다려야한다 -> 이전 작업이 끝날때까지 멈춰야한다.
// 논블로킹: 이전작업과 무관하게 작업이 진행된다, 멈추지 않는다.

//블로킹과 동기방식으로 계속 코드를 써왔다.

//싱글 스레드는 하나의 작업만을 진행하는데 어떻게 논브로킹이 성립되는가?
// 답은 시분할(라운드 로빈)
// 시분할 : 시(간)분할, 코드를 일정 시간으로 짤라서 실행한다.
// 여러 작업을 할때 각 코드에 대해서 일정한 시간 동안 진행 후 다른 코드를 진행한다.
//한번 다돌린후에 다시 다시 이어서 진행한다.

// REST API Server(응답해야들어옴?) , Socket Server(클라이언트와 서버가 주고받음 응답을 하지않았는데도 들어옴)
