...
contract SaleToken {
  ...
  function SalesToken(uint _tokenId, uint _price) public {
    ...
    // msg.sender 의 대리인이 address(this)인지 확인 (모든 NFT에 대한 대리인)
    // -> address(this)는 opensea다
    // 소유주의 토큰에 대해서 대리인이 모든 권한을 갖고있는지 확인
    // NFT에 대한 권한이 현재 컨트렉트에 있는가?
    // OpenSea를 기준으로 했을때 setApprovedForAll 메서드가 이미 있다.
    //setApprovedForAll: msg.sender가 _operator에게 자신이 소유한 모든 NFT에 대한 권한 위임

    // -메타마스크를 연결했을때 / 로그인했을때 => 메타마스크의 계정에대해 권한을 위임 받는다(서명)-> 컨트랙트(opensea)가 메타마스크계정이 갖고있는 코인에대한 권한을 위임받음
