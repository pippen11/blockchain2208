...
# 구현된 코드 상에서 서버 배포의 흐름(index.js파일)
...
flowchart
    ...
    J[static 함수를 호출하여 전역 변수에 미리 정의한다. / 12\n이거하면 staticFunc파일 폴더 합쳐주는과정?돌고 F로] --> E
    D --> |No| E
    E[TCP 서버를 생성한다. / 14\n없으면 global.isStatic안걸리고 !isStatic if문으로감] --> F
    F[클라이언트와 TCP 서버에 연결이 생겨서 데이터가 들어올 경우 실행하는 이벤트 메서드를 추가한다. / 15-41] --> G
    G[클라이언트에서 TCP 서버 연결을 끊을 경우 실행하는 이벤트 메서드를 추가한다. / 43-45\n서버가 응답을 준걸 http통신에서 끊는다=>클라이언트에서 TCP 서버 연결을 끊을 경우] --> H
