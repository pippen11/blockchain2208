...
const data = [1, 2, 3, 4];
//홀수개일때는 원래는 같은 데이터를 두번쓰게된다 두개를 더해서 암호화를 하게됨
//머클라이브러리는 데이터 홀수개짜리가 그대로 내려감  그래서 부전승방식으로 처리를 할거다
...
const createMerkleRoot = () => {
  ...
  //data[0].tostring으로 넣으면 정상작동 위에 라이브러리가 문자열로 처리해서 값은 1인 숫자라서 다른내용의 데이터를 받았다고 인식
  // SHA256(data[0]) < 데이터를 암호화한다. 해당 암호화된 데이터는 객체 형식이다
  // .toString() << 문자열로 변환한다.
  // .toUpperCase() << SHA256를 쓸경우 영어가 소문자로 나타나기 때문에 대문자로 수정해준다.
};
...
//데이터의 모든 정보(아이템)을 한번식 hash(SHA256) 방식으로 암호화한다

const secondTreeRoot = SHA256(firstTree[0] + firstTree[1])
  //더해서 암호화
  .toString()
  .toUpperCase();
// 데이터(data)안의 2개의 아이템을 암호화 한 후 연결해서 다시 암호화 하며 한개의 문자열로 만든다.

const secondTree = [];
for (let i = 0; i < data.length; i += 2) {
  secondTree.push(
    SHA256(firstTree[i].toString() + firstTree[i + 1])
      .toString()
      .toUpperCase()
  );
}
